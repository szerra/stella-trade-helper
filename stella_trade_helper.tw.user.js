...
// @version      1.6.31
// @description  台灣版：修正更新來源，確保台灣插件會抓取台灣版檔案；保留大陸推估欄位顯示與同數量新時間更新。
// @author       YourName
// @homepageURL  https://github.com/szerra/stella-trade-helper
// @updateURL    https://raw.githubusercontent.com/szerra/stella-trade-helper/main/stella_trade_helper.tw.user.js
// @downloadURL  https://raw.githubusercontent.com/szerra/stella-trade-helper/main/stella_trade_helper.tw.user.js
...
(() => {
  ...
  console.log('[StellaTrade 台灣版 1.6.31] 腳本已載入');

  const API_URL = 'https://script.google.com/macros/s/AKfycbxhW7OYwbkUIn--vH1unT_DhHV7Gbkw-72qTcHRPnWWoiYaV8-4wYQ4Q4TaKSBdBnAc4w/exec';
  ...
  function defaultInfo() {
    return {
      ...
      estimateBasis: '',
      estimateText: '',
      cnEstimatedRestockAt: '',
      cnEstimateStatus: 'unknown',
      cnRestockAnchorAt: '',
      cnRestockAnchorCount: '',
      cnRestockAnchorMax: '',
      cnEstimateBasis: '',
      cnDataTime: ''
    };
  ...
  function cloudField(info, oldInfo, key, fallback = '') {
    if (info && Object.prototype.hasOwnProperty.call(info, key)) return info[key] ?? fallback;
    if (oldInfo && Object.prototype.hasOwnProperty.call(oldInfo, key)) return oldInfo[key] ?? fallback;
    return fallback;
  }
    ...
    for (const [rawPort, items] of Object.entries(source)) {
      ...
      for (const [rawItem, info] of Object.entries(items)) {
        ...
        cleaned[port][item] = {
          ...
          cnEstimatedRestockAt: safe.cnEstimatedRestockAt || '',
          cnEstimateStatus: safe.cnEstimateStatus || 'unknown',
          cnRestockAnchorAt: safe.cnRestockAnchorAt || '',
          cnRestockAnchorCount: safe.cnRestockAnchorCount || '',
          cnRestockAnchorMax: safe.cnRestockAnchorMax || '',
          cnEstimateBasis: safe.cnEstimateBasis || '',
          cnDataTime: safe.cnDataTime || ''
        };
      }
    }
          ...
          for (const [port, items] of Object.entries(cloudData || {})) {
            ...
            for (const [item, info] of Object.entries(items || {})) {
              ...
              const incomingInfo = {
                ...
                cnEstimatedRestockAt: cloudField(info, oldInfo, 'cnEstimatedRestockAt'),
                cnEstimateStatus: cloudField(info, oldInfo, 'cnEstimateStatus', 'unknown'),
                cnRestockAnchorAt: cloudField(info, oldInfo, 'cnRestockAnchorAt'),
                cnRestockAnchorCount: cloudField(info, oldInfo, 'cnRestockAnchorCount'),
                cnRestockAnchorMax: cloudField(info, oldInfo, 'cnRestockAnchorMax'),
                cnEstimateBasis: cloudField(info, oldInfo, 'cnEstimateBasis'),
                cnDataTime: cloudField(info, oldInfo, 'cnDataTime')
              };
              ...
              mergedInfo.cnEstimatedRestockAt = cloudField(info, oldInfo, 'cnEstimatedRestockAt');
              mergedInfo.cnEstimateStatus = cloudField(info, oldInfo, 'cnEstimateStatus', 'unknown');
              mergedInfo.cnRestockAnchorAt = cloudField(info, oldInfo, 'cnRestockAnchorAt');
              mergedInfo.cnRestockAnchorCount = cloudField(info, oldInfo, 'cnRestockAnchorCount');
              mergedInfo.cnRestockAnchorMax = cloudField(info, oldInfo, 'cnRestockAnchorMax');
              mergedInfo.cnEstimateBasis = cloudField(info, oldInfo, 'cnEstimateBasis');
              mergedInfo.cnDataTime = cloudField(info, oldInfo, 'cnDataTime');
  ...
  function stripEstimateBasis(text) {
    return String(text || '').replace(/（[^）]*）/g, '').trim();
  }

  function shortTaiwanEstimateText(info) {
    const estimatedAt = toTimestamp(info?.estimatedRestockAt);
    const status = String(info?.estimateStatus || '');
    const basis = String(info?.estimateBasis || '').trim();
    ...
    if (shouldShowEstimate) {
      const text = formatClock(new Date(estimatedAt), new Date());
      return `${estimatedAt < Date.now() ? '已過' : '約'} ${text}`;
    }

    const cloudText = stripEstimateBasis(info?.estimateText || info?.restockEstimateText || info?.estimatedRestockText || '');
    if (cloudText && cloudText !== '-' && cloudText !== '資料不足' && cloudText !== '尚未售罄') return cloudText;

    return '-';
  }

  function isValidCnEstimate(info) {
    const estimatedText = String(info?.cnEstimatedRestockAt || '').trim();
    if (!estimatedText || estimatedText === '-') return false;

    const status = String(info?.cnEstimateStatus || '').trim().toLowerCase();
    ...
    return !!toTimestamp(estimatedText);
  }

  function shortCnEstimateText(info) {
    if (!isValidCnEstimate(info)) return '-';

    const estimatedAt = toTimestamp(info?.cnEstimatedRestockAt);
    const text = formatClock(new Date(estimatedAt), new Date());
    return `${estimatedAt < Date.now() ? '已過' : '約'} ${text}`;
  }
  ...
  function renderPortItemRow(itemName, info, change, settings) {
    ...
    const twEstimate = shortTaiwanEstimateText(info);
    const cnEstimate = shortCnEstimateText(info);
    const twEstimateLine = twEstimate === '-' ? '推估：-' : `推估：台灣 ${twEstimate}`;

    return `
      <div class="stella-good-row ${low ? 'low' : ''}">
        <div class="stella-good-main">
          <div class="stella-good-name">${escapeHtml(itemName)}</div>
          <div class="stella-good-meta">更新：${escapeHtml(info.time || '尚未更新')}　補貨：${escapeHtml(info.restock || '-')}　${escapeHtml(twEstimateLine)}　大陸：${escapeHtml(cnEstimate)}</div>
        </div>
          ...
          ${entries.map(([itemName, info]) => {
            ...
            return `
              <div class="stella-detail-good">
                ...
                <div class="stella-detail-meta">更新：${escapeHtml(info.time || '尚未更新')}　補貨：${escapeHtml(info.restock || '-')}　${escapeHtml(twEstimateLine)}　大陸：${escapeHtml(cnEstimate)}</div>
              </div>
            `;
